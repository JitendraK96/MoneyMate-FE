                ...
                >
                  ...
                  <div className="flex items-center gap-1 min-w-0 flex-1">
                    <span className="font-size-extra-small w-6 text-right">
                      {allocation.allocated_percentage.toFixed(1)}%
                    </span>
...
export const getEnhancedIncomeColumns = (
    ...
    cell: ({ row }) => {
      ...
      return (
        <div>
          ...
          {description && (
            <div className="font-size-extra-small text-gray-500 truncate max-w-[200px] mt-1">
              {description}
            </div>
          )}
        </div>
  ...
  },
  {
    ...
    header: () => <div className="text-left">Enhanced Budget Allocation</div>,
    cell: ({ row }) => {
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const hasCategoryAllocations =
        (income.needs_category_allocations?.length || 0) > 0 ||
        (income.wants_category_allocations?.length || 0) > 0 ||
        (income.savings_category_allocations?.length || 0) > 0;
      ...
      return (
        <div className="flex flex-col gap-1">
          ...
          {isActive && hasCategoryAllocations && (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleDelete = (e: React.MouseEvent) => {
        ...
        e.stopPropagation();
        if (
          window.confirm(`Are you sure you want to delete "${income.source}"?`)
        ) {
          onDelete(income.id);
        }
      };
...
export default getEnhancedIncomeColumns;
